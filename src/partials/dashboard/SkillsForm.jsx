...
function SkillsForm() {
  ...
  return (
    // here is layout
    <div className="flex flex-col col-span-12 xl:col-span-6 bg-white dark:bg-slate-800 shadow-lg rounded-sm border border-slate-200 dark:border-slate-700">
      <form
        className="p-6 px-8"
        onSubmit={(e) => {
          e.preventDefault();
      ...
      >
        <div className="mb-4">
          <label className="mb-2 py-2">請輸入學號</label>
          <input
            type="text"
        ...
        {Object.keys(skills).map((skill) => (
          <div className="" key={skill}>
            <label className="px-3 my-2">{skill}</label>
            <select
        ...
        <button
          type="submit"
          className="bg-school text-white px-6 py-2 rounded mt-6"
        >
          Submit
        </button>
      </form>
